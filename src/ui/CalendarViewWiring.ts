import { CachedMetadata, Component, TAbstractFile, TFile } from "obsidian";
...
export function makeEventSourceFunc(
...
export function registerVaultChangesToCalendarUpdates(
	plugin: HorizonCalPlugin,
	componentLifetime: Component,
	cal: CalendarApi,
) {
	// So about events.
	...
	componentLifetime.registerEvent(
		plugin.app.vault.on(
			"rename",
		...
		plugin.app.metadataCache.on(
			...
			(file: TFile, data: string, cache: CachedMetadata) => {
				...
				if (fcEvt == null) {
					...
					//   so that fullcalendar will do ID-based dedup as one might expect.
					cal.addEvent(hcEvt.toFCdata(plugin.settings), "horizoncal");
				} else {
...
type CalendarChangeHook = (info: EventDropArg | EventResizeDoneArg) => void;

export function makeCalendarChangeToVaultUpdateFunc(
	plugin: HorizonCalPlugin,
): CalendarChangeHook {
		...
		await plugin.app.fileManager.processFrontMatter(
			...
			(fileFm: any): void => {
				...
				const newEndDt = toLuxonDateTime(
					...
					hcEvt.endTZ.valueStructured || hcEvt.evtTZ.valueStructured,
				);
		...
		if (wholePath != info.event.id) {
			...
			try {
				await plugin.app.fileManager.renameFile(
					file,
					`${plugin.settings.prefixPath}/${wholePath}`,
				);
			} catch (error) {
				alert(
					"Error: could not move event file -- " +
						error +
			...
			info.event.setProp(
				"id",
				`${plugin.settings.prefixPath}/${wholePath}`,
			); // FIXME canonicalization check, double slash would be bad here.
		}
